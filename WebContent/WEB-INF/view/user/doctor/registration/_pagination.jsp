	...
	<c:if test="${pageCount > 1}">
		<div class="dataTables_paginate paging_full_numbers" id="dynamic_paginate">
			<c:if test="${(selPage-1)>=0}">
				<a tabindex="0" href="${baseURL}/user/doctor/registration?page=${selPage-1}" class="previous paginate_button paginate_button_disabled" id="dynamic_previous">Previous</a>
			</c:if>
			<span>
			...
			<c:if test="${(selPage+1)<=(pageCount-1)}">
				<a tabindex="0" href="${baseURL}/user/doctor/registration?page=${selPage+1}" class="next paginate_button paginate_button_disabled" id="dynamic_next">Next</a>
			</c:if>
		</div>
	</c:if>
</div>
