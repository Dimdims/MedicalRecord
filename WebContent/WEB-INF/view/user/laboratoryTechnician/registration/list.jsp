...
<body>
	...
	<table  border="1" bordercolor="#dfdfdf" class="static_table resizable">
	 <thead>
		 ...
		 <th>Prescription Sp</th>
		 <th>Laboratory Technician</th>
		 <th>Laboratory Report</th>
		 <th>Laboratorium File</th>
		 <th>Date</th>
		 <th>Action</th>
		 </tr>
	 </thead>
	...
	<td>
		<c:if test="${not empty registration.patient}">
			${registration.patient.username } (click <a href="${baseURL}/user/laboratoryTechnician/registration/medicalHistory/${registration.patient.id}">here</a> for his medical histories)
		</c:if>
	...
	<td>
		<c:if test="${not empty registration.laboratoriumReport && not empty registration.laboratoriumReport.fileUrl}">
			<img src="${baseURL}/laboratorium-folder/${registration.laboratoriumReport.fileUrl}"/>
		</c:if>
	</td>
	<td><fmt:formatDate pattern="dd MMMM yyyy" type="date" value="${registration.createdDate}" /></td>
	<td><a href="${baseURL}/user/laboratoryTechnician/registration/edit/${registration.id}">edit</a></td>
	</tr>
	</c:forEach>
	 </tbody>
	</table>
